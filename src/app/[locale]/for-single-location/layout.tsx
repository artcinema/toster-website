...
export default async function ForSingleLocationLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  return (
    <>
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(buildBreadcrumbSchema(locale, [{ name: 'For Single Locations', path: '/for-single-location' }])),
        }}
      />
      {children}
    </>
